...
\begin{columns}[T,onlytextwidth]
  \column{0.5\textwidth}
  \begin{block}{What Faculty Expect}
    \begin{itemize}\setlength{\itemsep}{3pt}
      \item Most faculty distribute and collect \textbf{Word (.docx)}, \textbf{Excel (.xlsx)}, and \textbf{PowerPoint (.pptx)} files
      \item Microsoft 365 remains the dominant platform in public-sector workplaces (roughly 80--85\%)
      \item Some faculty also use \textbf{Google Docs} for drafting, feedback, and AI-policy transparency
    \end{itemize}
  \end{block}

  \column{0.5\textwidth}
  \begin{block}{What Matters Professionally}
    \begin{itemize}\setlength{\itemsep}{3pt}
      \item You are \emph{not} required to use one platform. Google Workspace, LibreOffice, and the Mac apps are all fine
      \item You \emph{are} expected to work across platforms: reliably export to standard Office formats when a course or workplace requires it
      \item And to open those files back up to read faculty comments and tracked changes
    \end{itemize}
  \end{block}
\end{columns}

\vspace{0.2em}
\centering\alert{\textbf{Draft anywhere. Submit in the format the assignment asks for.}}
\end{frame}
